...
const changelog: ChangelogEntry[] = [
  {
    ...
    type: 'feature',
    highlights: [
      '新增 SkillExtractionService（419行）：节点执行完成后自动评估产出物价值，高置信度结果沉淀为 SKILL.md',
      '百分制评分引擎 5 维指标：节点类型权重（0~30）、内容长度（0~15）、Markdown 结构化（0~20）、代码块密度（0~15）、关键词匹配（0~20）',
      'Jaccard 词集相似度去重：与项目已有 Skills 对比，相似度 >0.7 自动跳过避免冗余',
      '项目级 Skill 存储：project.path/.agent-flow/skills/<slug>/SKILL.md，含 YAML frontmatter，随项目版本控制',
      '事件驱动集成：run:node_updated status=completed 触发异步 extractFromNode()，不阻塞主流程',
      '手动沉淀 + 自动沉淀双模式：forceExtract() 跳过评分直接持久化（置信度 1.0）',
      'SkillService 扩展 5 个方法：reload / loadAdditional / writeSkill / getSkillById / unregisterSkill',
      'ProjectService 扩展：.agent-flow/skills 加入搜索路径首位 + getSkillsDir() 获取项目 Skill 目录',
      '新增 4 条 REST API：extraction-stats / extraction-log / extract / project-dir',
      '修复路由优先级：/skills/:name 通配参数路由移至末尾，避免拦截具名路由',
    ],
...
编译验证：Client 和 Server tsc --noEmit 均 0 错误。`,
  },
  {
    ...
    type: 'feature',
    highlights: [
      '新增 PR 模式：Agent 产出代码 Review 后可直接推送分支并创建 GitHub Pull Request',
      '项目设置新增"代码合入方式"配置：支持 Local Merge 和 PR 模式切换',
      '仓库类型自动检测：通过 Owner 类型、Collaborators、Commit 作者多样性、Branch Protection 四维度加权评分',
      '团队项目强制 PR 模式：检测为 Organization 或多人协作仓库时自动锁定为 PR 模式，禁止切换回 local',
      'DiffReviewPanel 根据 mergeMode 动态展示"Approve & Merge"或"创建 PR"按钮',
      'PR 创建成功后显示可点击链接，直达 GitHub PR 页面',
    ],
    details: `v2.8.3 为团队协作场景引入了完整的 GitHub PR 工作流，确保多人项目的代码合入必须经过 Pull Request 审查。
...
【前端改造】DiffReviewPanel 根据项目 mergeMode 动态展示：local 模式显示合入策略选择 + "Approve & Merge"；PR 模式显示"创建 PR"按钮，成功后变为可点击的 PR 链接。SettingsPanel 新增"代码合入方式"折叠面板，进入时自动检测仓库类型并展示检测结果卡片。`,
  },
  {
    version: 'v2.8.2',
    date: '2026-06-03',
...
【测试增强】Vitest 单元测试从 68 cases 增长到 122 cases，新增三个测试套件：dag-scheduler.test.ts（拓扑排序、并行节点、条件分支）、turn-manager.test.ts（Turn 生命周期、并发安全）、storage-sqlite.test.ts（CRUD、迁移、事务回滚）。确保重构后所有行为与原实现一致。`,
  },
  {
    ...
    type: 'feature',
    highlights: [
      '多用户远端仓库结构（users/{github_login}/ + shared/）',
      'gitRemote 字段自动检测（添加项目时 git remote get-url origin）',
      'normalizeGitRemote() SSH/HTTPS URL 标准化匹配',
      'mergeProjects() 跨设备项目智能匹配（gitRemote 优先 + pathMapping 兜底）',
      'replaceProjectId() 本地临时 ID → 远端全局 ID 自动替换',
      '新增 4 条 API 路由（remote-projects + path-mapping CRUD）',
      'ProjectData 新增 gitRemote 字段',
    ],
    details: `v2.7.2 将数据同步从单用户模式升级为多用户隔离架构，并实现了跨设备项目自动匹配，解决"新电脑 Pull 后项目路径对不上"的问题。
...
【使用流程】用户只需：① 在新设备 clone 项目代码 → ② 在 AgentFlow 中添加项目（自动检测 gitRemote）→ ③ Pull 同步数据（自动匹配并替换 ID），全程无需手动配置路径映射。`,
  },
  {
    version: 'v2.7.1',
    date: '2026-06-01',
...
【设计决策 ADR-016】明确拒绝"完整自演进系统"（自动发现→自动决策→自动执行→自动验证），选择轻量采集 + 人工决策模式。核心原则：数据采集类服务只收集不决策，决策权归用户；系统改进的执行方式是"用户 + AI 对话"，不是系统自动修改自身代码。`,
  },
  {
    ...
    type: 'feature',
    highlights: [
      'Diff Review 面板（GitHub PR 风格：文件树 + 行级 Diff + Approve/Discard）',
      'ArtifactMergeService（Git worktree diff + squash/merge/rebase 三种合并策略）',
      'Metrics 指标面板（Overview / Timeline / Token 分布 / 效率评分四子视图）',
      'MetricsCollector（时间/Token/质量全链路采集 + 持久化 JSON 存储）',
      '新增 4 条 API 路由（diff-review CRUD + metrics 查询）',
      'RunDetailTab 扩展支持 diff-review / metrics 视图',
    ],
    details: `v2.6.0 实现了第四优先级的两大核心能力：产出物闭环和可观测性增强，使 AgentFlow 从"能执行"进化到"可审可控可度量"。
...
【API 路由扩展】新增 GET /api/runs/:id/diff-review（获取 Diff 数据）、POST /api/runs/:id/diff-review/merge（执行合并）、POST /api/runs/:id/diff-review/discard（丢弃变更）、GET /api/runs/:id/metrics（获取指标数据）。RunDetailTab 新增 'diff-review' 和 'metrics' 两个 Tab 入口。`,
  },
  {
    version: 'v2.5.0',
    date: '2026-05-31',
...
【A2A 消息面板】Run 详情页新增 A2A 消息 Tab，提供三种视图全面展示 Agent 间通信：拓扑图视图以纯 SVG 环形布局呈现 Agent 网络关系（节点按角色着色、连线粗细反映消息量）；时间线视图按时间倒序展示消息列表（支持类型/优先级筛选、展开 payload 详情）；统计视图展示量化指标卡片 + 消息类型分布柱状图 + Agent 活跃度排行。前端 a2aApi 封装对接后端 /api/a2a/* 全部路由。`,
  },
  {
    ...
    type: 'fix',
    highlights: [
      '修复审批后需刷新才能看到下一节点的 Bug',
      'computeReadyNodes 状态变更广播 WS 事件',
      'Agents 页面 Token 统计改为后端持久化拉取',
      'Token 数据刷新页面不再丢失',
    ],
    details: `v2.4.3 修复了两个影响使用体验的关键问题。
...
【Token 统计持久化】Agents 页面的 Token 消耗总览之前仅依赖浏览器内存中的 WebSocket 实时事件，刷新页面后数据丢失。修复后改为页面加载时从后端 /runs/:id/token-stats API 拉取所有 Run 的持久化 token 数据，合并实时 WS 数据一起展示，确保统计数据不因刷新而消失。`,
  },
  {
    version: 'v2.4.2',
    date: '2026-05-30',
