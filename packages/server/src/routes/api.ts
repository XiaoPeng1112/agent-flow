...
export function createApiRouter(deps: {
  ...
  // ════════════════════════════════════════
  // Artifact Merge API (产出物闭环)
  // ════════════════════════════════════════

  /** 为节点生成 Diff Review（在 wait_user_review 时调用） */
  router.post('/runs/:runId/nodes/:nodeId/diff-review', (req, res) => {
  ...
  router.get('/runs/:runId/nodes/:nodeId/diff-review', (req, res) => {
  ...
  /** 获取指定文件的详细 Diff */
  router.get('/runs/:runId/nodes/:nodeId/diff-review/:turnId/file', (req, res) => {
  ...
  /** Approve 后合入工作分支 */
  router.post('/runs/:runId/nodes/:nodeId/merge', (req, res) => {
  ...
  /** Reject/Skip 时丢弃工作分支 */
  router.post('/runs/:runId/nodes/:nodeId/discard', (req, res) => {
    const { turnId } = req.body
    if (!turnId) {
      res.status(400).json({ success: false, error: 'turnId is required' })
      return
  ...
  /** 获取 Token 分布 */
  router.get('/runs/:runId/metrics/token-distribution', (req, res) => {
    const { runId } = req.params
    try {
      // 确保先计算指标
  ...
  /** 获取效率表格 */
  router.get('/runs/:runId/metrics/efficiency', (req, res) => {
    const { runId } = req.params
    try {
      if (!metricsCollector.getRunMetrics(runId)) {
        const run = workflowEngine.getRun(runId)
